...
namespace astar_planner{
    ...
    bool AstarPlanner::makePlan(const geometry_msgs::PoseStamped& start,
        ...
        if (hypot(start.pose.position.x - goal.pose.position.x, start.pose.position.y - goal.pose.position.y)<(0.3)) {
            if (!initialized_) {
            ...
            while (!openlist.empty()) {
                ...
                if (getHeuristic(currentNode.index, goal_index) < 4) {
                    ROS_INFO("Got Goal");
                    indx = goal_index;
                ...
                for (int i = 0; i < trajectory.size(); ++i) {
                    ...
                    it = find_if(openlist.begin(), openlist.end(), [nextNode](Node node) {
                        return (node.index == nextNode.index);
                    });
...
/*
    ...
    Eigen::Isometry3d AstarPlanner::getSe3(geometry_msgs::PoseStamped current_pose_) {
    ...
    Eigen::Isometry3d AstarPlanner::discretise(Eigen::Isometry3d& pose){
    ...
    bool AstarPlanner::trajectoryGen(vector<vector<Eigen::Isometry3d>> &pose){
        vector<double> angle;
    ...
    bool AstarPlanner::getTrajectory(const Eigen::Isometry3d &pose, vector<double>& v_window, vector<double>& w_window, vector<double>& movingcost, vector<vector<Eigen::Isometry3d>>& trajectory){
        double dt = 0.03;
    ...
    double AstarPlanner::getVP(double ang, double chordl, double range){
    ...
    double AstarPlanner::getVN(double ang, double chordl, double range){
        double angle = fabs(ang);
        if(angle==0){
            return chordl/(range/2/0.26);
        }
    ...
    bool AstarPlanner::refine(vector<vector<Eigen::Isometry3d>>& trajectory){
    ...
    double AstarPlanner::getHeuristic(const Eigen::Isometry3d& pose,const Eigen::Isometry3d& goal, double v, double w)
    {
        double dt = 0.03;
